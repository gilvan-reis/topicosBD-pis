  ...
  public int run(String[] args) throws Exception {
    ...
    while(br.ready()){
      ...
      for(int i = 0; i < valor.length; i++){
        bytes[i] = new Byte(valor[i]);
        System.out.println(valor[i] +" : "+bytes[i]);
      }
      System.out.println(Byte.decode(valor[1]));

      PairOfWritables<IntWritable, ArrayListWritable<PairOfInts>> postings = PostingReader.readPostings(new BytesWritable(bytes));
